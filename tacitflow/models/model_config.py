...
By default TacitFlow uses a local Ollama runtime and the Gemma model family. It must NOT
call cloud LLM APIs. Configuration is set with ``tacitflow config set model.* ...``.
"""
...
class ModelConfig(BaseModel):
    ...
    def assert_local(self) -> None:
        ...
        if self.provider != "ollama":
            raise ValueError(
                f"TacitFlow defaults to a local Ollama runtime; provider={self.provider!r} "
...
def project_home(explicit: str | None = None) -> Path:
    ...
    return Path(os.environ.get("TACITFLOW_HOME", Path.cwd() / ".tacitflow"))
