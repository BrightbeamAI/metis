...
def run_spec(spec: ScenarioSpec, engine: TacitFlowEngine | None = None, *,
             use_live_model: bool = False) -> DemoRun:
    engine = engine or TacitFlowEngine(workspace_id=spec.workspace_id, name=spec.name,
    ...
    s("1. TacitFlow workspace created.", engine.adapter.workspace_id)
...
def run_manufacturing(engine: TacitFlowEngine | None = None, *, use_live_model: bool = False) -> DemoRun:
    return run_spec(MANUFACTURING, engine, use_live_model=use_live_model)


def run_batch_quality(engine: TacitFlowEngine | None = None, *, use_live_model: bool = False) -> DemoRun:
    return run_spec(BATCH_QUALITY, engine, use_live_model=use_live_model)


def run_shift_handover(engine: TacitFlowEngine | None = None, *, use_live_model: bool = False) -> DemoRun:
