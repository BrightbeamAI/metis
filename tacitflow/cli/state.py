"""CLI session state: persist a TacitFlow run so later commands can query it.

The CHAP evidence chain is exported to ``.tacitflow/evidence.jsonl`` (the audit source of
...
def save_state(engine, *, scenario: str | None = None) -> Path:
    ...
    try:
        ...
        store = SqliteStore(home() / "tacitflow.db")
...
def load_state() -> dict[str, Any]:
    if not state_path().exists():
        raise FileNotFoundError(
            "No TacitFlow state found. Run `tacitflow demo manufacturing-pump-vibration` first.")
